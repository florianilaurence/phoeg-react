...
export default function PolytopeChart(props) {
  ...
  const directions = ["minY", "minXminY", "minX", "maxXminY", "maxX", "maxXmaxY", "maxY", "minXmaxY"];
      ...
      return (
        <View style={{
          ...
          {tagsGradient.map((tag, i) => <Text
            onPress={() => handleOnPressLegend(tag)}
            style={tag === selectedTag ? {
                fontsize: INNER_TEXT_SIZE, color: colorsGradient[i],
  ...
  const constructPoints = () => {
    ...
    props.allClusters[currentNbClusters].forEach((group, i) => {
      ...
      group.forEach((currentData, j) => {
        ...
        result.push({
          key: `(x:${x};y:${y};col:${col};index:${j})`, x: x, y: y, r: 3, mult: mult, col: col,
  ...
  const RenderData = () => {
    return (
      <Group left={margin.left} top={margin.top}>
        <AxisLeft scale={yScale} left={margin.left} label={context.invariantY}/>
        <AxisBottom scale={xScale} top={innerHeight} label={context.invariantX}/>
        ...
        <g>
          ...
          />
          <LinePath stroke={DirectionColors.minY} strokeWidth={2} data={props.concave.minY}
                    x={(d) => xScale(accessors(d, "x"))}
                    y={(d) => yScale(accessors(d, "y"))}
          />
          <LinePath stroke={DirectionColors.minXminY} strokeWidth={2} data={props.concave.minXminY}
                    x={(d) => xScale(accessors(d, "x"))}
                    y={(d) => yScale(accessors(d, "y"))}
          />
          <LinePath stroke={DirectionColors.minX} strokeWidth={2} data={props.concave.minX}
                    x={(d) => xScale(accessors(d, "x"))}
                    y={(d) => yScale(accessors(d, "y"))}
          />
          <LinePath stroke={DirectionColors.minXmaxY} strokeWidth={2} data={props.concave.minXmaxY}
                    x={(d) => xScale(accessors(d, "x"))}
                    y={(d) => yScale(accessors(d, "y"))}
          />
          <LinePath stroke={DirectionColors.maxY} strokeWidth={2} data={props.concave.maxY}
                    x={(d) => xScale(accessors(d, "x"))}
                    y={(d) => yScale(accessors(d, "y"))}
          />
          <LinePath stroke={DirectionColors.maxXmaxY} strokeWidth={2} data={props.concave.maxXmaxY}
                    x={(d) => xScale(accessors(d, "x"))}
                    y={(d) => yScale(accessors(d, "y"))}
          />
          <LinePath stroke={DirectionColors.maxX} strokeWidth={2} data={props.concave.maxX}
                    x={(d) => xScale(accessors(d, "x"))}
                    y={(d) => yScale(accessors(d, "y"))}
          />
          <LinePath stroke={DirectionColors.maxXminY} strokeWidth={2} data={props.concave.maxXminY}
                    x={(d) => xScale(accessors(d, "x"))}
                    y={(d) => yScale(accessors(d, "y"))}
          />
          {props.allClusters[currentNbClusters] ? constructPoints().map(circle => {
            return (
              <Tooltip
                ...
                title={
                  context.invariantX.replace('_', ' ') + " = " + circle.x + " | " +
                  context.invariantY.replace('_', ' ') + " = " + circle.y + " | " +
                  "mult = " + circle.mult
                }>
                <circle
                  className="circle"
  ...
  const RenderLegend = () => {
    return (
      <View>
        {context.invariantColor !== "num_vertices" ?
          <>
            <View style={{
              marginTop: TOP, marginLeft: LEFT, marginRight: RIGHT, paddingBottom: BOTTOM,
              maxWidth: '100%',
            }}>
              <Grid container>
                <Grid item xs={3}>
              ...
              <View style={{
                ...
                <InnerText>Legend (coloration with {context.invariantColor}): </InnerText>
          ...
          <>
            <View style={{marginTop: INNER, marginBottom: INNER, paddingLeft: LEFT, paddingRight: RIGHT}}>
              ...
              <View style={{flexDirection: 'row', textAlign: 'center'}}>
                ...
                <InnerText bold>{context.invariantX}= </InnerText>
                <InnerText bold italic>{selectedX}</InnerText>
                <InnerText>, y: </InnerText>
                <InnerText bold>{context.invariantY}= </InnerText>
  ...
  return (
    <View style={{marginBottom: INNER}}>
      ...
      <View>
        {directions.map((key) => {
          return (
            <Text style={{color: DirectionColors[key]}}>
              {key}
            </Text>)
        })}
      </View>
