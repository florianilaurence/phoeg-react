...
export default function PolytopeForm({invariantsAxis, invariantsColoration, invariantsConstraint, types}) {
  const [, updateState] = useState();
  const forceUpdate = useCallback(() => updateState({}), []);
  const [checked, setChecked] = useState(false);
  const [formData, setFormData] = useState({
    order: DEFAULT_ORDER,
    invariantX: invariantsAxis[0],
    invariantY: invariantsAxis[1],
  ...
  const getTypeFromName = (name) => {
    let type = "";
    for (let i = 0; i < invariantsConstraint.length; i++) {
      if (invariantsConstraint[i].label === name) {
        type = types[i];
        break;
      }
    }
    ...
    return (
      <Box height='125px' m={1} pt={1} sx={{
        ...
        <Box m={1} pt={1} sx={{
          ...
          <AdvancedConstraintsInput ref={advancedConstraintsRef} text={formData.advancedConstraints}
                                    onChange={handleChangeAdvancedConstraint}/>
        </Box>
      </Box>
    )
  };
